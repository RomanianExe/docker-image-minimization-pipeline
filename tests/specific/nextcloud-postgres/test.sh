#!/usr/bin/env bash
# Functional test suite for the "nextcloud-postgres" example.
...
DB_CONTAINER="dip-nextcloud-postgres-db-1"
...
docker exec "$DB_CONTAINER" pg_isready -U nextcloud
...
echo "All nextcloud-postgres functional tests passed."
